...
async def handle_unix_client(reader, writer):
    logger.info("Novo motor conectado ao TCP 9000 (V1).")
    try:
        while True:
        ...
        logger.info("Motor desconectado do TCP Socket.")
...
async def lifespan(app: FastAPI):
    ...
    logger.info("Servidor IPC TCP iniciado em 127.0.0.1:9000")
    
    yield
    
    # Shutdown: Parar o servidor
    server.close()
    await server.wait_closed()
    logger.info("Servidor IPC TCP encerrado.")

app = FastAPI(title="SPECTRE_GRID NGFW Dashboard V1", lifespan=lifespan)
...
if __name__ == "__main__":
    uvicorn.run("dashboard_api_benchmark:app", host="0.0.0.0", port=8000)
