...
$simJob = Start-Process wsl -ArgumentList "-d Ubuntu -e bash -c `"cd tools && source ../.venv_wsl/bin/activate && python3 traffic_simulator.py`"" -NoNewWindow -PassThru
...
if ($apiRunning) {
    ...
    Write-Host "Open in Browser: http://localhost:8002/static/index.html" -ForegroundColor Green
    Write-Host "WebSocket Endpoint: ws://localhost:8002/ws/threats" -ForegroundColor Green
    Write-Host "======================================================" -ForegroundColor Cyan
    ...
    Write-Host "Stop-Process -Id $($apiJob.Id), $($simJob.Id) -Force" -ForegroundColor Red
} else {
