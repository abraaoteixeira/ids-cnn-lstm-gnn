...
def build_graph(
    ...
    if src_col and dst_col:
        ...
        node_target_sum = defaultdict(int)
        node_target_count = defaultdict(int)
        ...
        for _, row in df_proc.iterrows():
            ...
            # node label: track target flow counts per node
            if target_col in df_proc.columns:
                t = _map_target_to_binary(pd.Series([row[target_col]])).iloc[0]
                node_target_sum[s_idx] += t
                node_target_sum[d_idx] += t
                node_target_count[s_idx] += 1
                node_target_count[d_idx] += 1

        for idx in range(num_nodes):
            if node_target_count[idx] > 0 and (node_target_sum[idx] / node_target_count[idx]) >= 0.22:
                node_labels[idx] = 1
...
def generate_test_sample(out_csv: Optional[str] = None, n: int = 100) -> pd.DataFrame:
    ...
    for i in range(n):
        ...
        target = 1 if np.random.rand() < 0.25 else 0
        feats = {f'f{j}': float(np.random.rand() + (2.5 if (target == 1 and j in [0, 1, 2, 3, 4]) else 0.0)) for j in range(20)}
        row = {
            'timestamp': ts,
            'src_ip': src,
            'dst_ip': dst,
