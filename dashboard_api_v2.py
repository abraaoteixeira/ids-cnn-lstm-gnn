...
SOCKET_PATH = os.getenv("SPECTRE_SOCKET_PATH", "/tmp/spectre.sock")
...
PFSENSE_ENABLED = os.getenv("PFSENSE_ENABLED", "false").lower() == "true"
pfsense_blocker = PfSenseBlocker()
        ...
        while True:
            ...
            if message:
                ...
                try:
                    ...
                    # Automatic pfSense firewall block integration
                    is_threat = payload.get("is_threat", False)
                    src_ip = payload.get("src_ip", "")
                    if is_threat and PFSENSE_ENABLED and src_ip:
                        loop = asyncio.get_running_loop()
                        logger.info(f"Threat detected from {src_ip}. Triggering asynchronous block on pfSense.")
                        loop.run_in_executor(None, pfsense_blocker.block_ip, src_ip)
...
# Ensure assets directory exists to prevent Starlette StaticFiles initialization crash
...
# ==============================================================================
# BORDER DEFENSE ROUTES (pfSense Integration)
# ==============================================================================
class DefenseIPRequest(BaseModel):
    ip: str
    interface: Optional[str] = None

@app.post("/api/defense/block")
async def manual_block(req: DefenseIPRequest):
    if not PFSENSE_ENABLED:
        return {"status": "error", "message": "pfSense integration is disabled."}
    
    loop = asyncio.get_running_loop()
    success = await loop.run_in_executor(None, pfsense_blocker.block_ip, req.ip, req.interface)
    
    if success:
        return {"status": "success", "message": f"IP {req.ip} blocked successfully on pfSense."}
    else:
        return {"status": "error", "message": f"Failed to block IP {req.ip} on pfSense."}

@app.post("/api/defense/unblock")
async def manual_unblock(req: DefenseIPRequest):
    if not PFSENSE_ENABLED:
        return {"status": "error", "message": "pfSense integration is disabled."}
        
    loop = asyncio.get_running_loop()
    success = await loop.run_in_executor(None, pfsense_blocker.unblock_ip, req.ip, req.interface)
    
    if success:
        return {"status": "success", "message": f"IP {req.ip} unblocked successfully on pfSense."}
    else:
        return {"status": "error", "message": f"Failed to unblock IP {req.ip} on pfSense."}
