...
*   **IPC via Unix Sockets (`/tmp/spectre.sock`):** Zera o I/O físico de disco removendo arquivos de log intermediários. O Daemon transmite os JSONs diretamente para a memória RAM do backend FastAPI ([dashboard_api_v2.py](dashboard_api_v2.py)) ou do Go Server ([main.go](dashboard_go/main.go)).
*   **Dashboard Web Enterprise (`dashboard_v2/`):** Interface profissional de monitoramento em tempo real inspirada nos painéis Cloudflare/Fortinet, desenvolvida com React 19, Vite e Recharts. Possui controle duplo de visualização no painel principal:
    *   **Nível de Ameaça (GNN) & Tráfego Real:** Gráfico de área que plota três métricas simultâneas: probabilidade de intrusão (vermelho), pesos de atenção (azul) e a taxa de tráfego consumida em KB/s (verde), acompanhado por um **Módulo de Auditoria de IA (XAI)** em split-screen. O HUD lateral exibe latência de inferência, classificação de criticidade (BAIXO, MÉDIO, ALTO, CRÍTICO) e o pipeline neural interativo.
    *   **Globo 3D Dinâmico & Grafo de Nós:** O Globo espacial renderiza conexões seguras como lasers verdes convergindo para o Nó Local, e ataques como lasers vermelhos. O Grafo 2D exibe a topologia relacional da rede (limite de 80 nós ativos e 150 links para estabilidade de memória).
*   **Localização Completa (PT-BR) & Identidade IFC:** Dashboard totalmente em Português do Brasil com integração oficial da logomarca do Instituto Federal Catarinense (IFC) (48px constraint) no cabeçalho.
*   **SQLite Storage (`spectre_history_v2.db`):** Registra o histórico persistente das ameaças mitigadas usando gravação assíncrona por lotes (batch inserts), com botão de limpeza de banco 100% funcional via API na porta 8002.
...
## 🌍 Exposição Pública e Acesso Remoto

A versão 2 do dashboard (`dashboard_v2`) possui arquitetura de roteamento dinâmico (buscando WebSockets via `window.location.host`), permitindo o acesso de qualquer dispositivo móvel através de túneis reversos.

Para expor o painel publicamente **sem necessidade de Port Forwarding** ou roteamento NAT, utilize um dos métodos de túnel:

### 1. Via LocalTunnel (Instalação Global NPM)
Gera uma URL gratuita baseada no subdomínio solicitado (ideal para testes rápidos).
```powershell
npx -y localtunnel --port 8002 --subdomain spectregrid
# Retorna: https://spectregrid.loca.lt
```

### 2. Via Cloudflare Tunnels (Zero Trust)
Oferece segurança Enterprise-grade. Exige o uso do utilitário `cloudflared`.
```powershell
# Túnel Rápido (Quick Tunnel Aleatório)
cloudflared tunnel --url http://localhost:8002

# Para URLs Fixas (ex: spectre.seudominio.com.br): 
# Configure o Public Hostname no Painel Cloudflare Zero Trust e aponte o Service para localhost:8002.
```
