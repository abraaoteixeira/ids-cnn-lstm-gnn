...
static void write_cpp_alert_to_log(uint32_t src_ip, uint32_t dst_ip, float prob, uint64_t bytes, uint64_t packets) {
    if (g_ipc_fd < 0) {
        ...
        if (g_ipc_fd >= 0) {
            ...
            const char* sock_path = std::getenv("SPECTRE_SOCKET_PATH");
            if (!sock_path) {
                sock_path = "/tmp/spectre.sock";
            }
    ...
    struct in_addr dst_addr; dst_addr.s_addr = dst_ip;
    ...
    ss << "{\"flow_id\": " << (std::rand() % 10000)
       ...
       << ", \"probability\": " << std::fixed << std::setprecision(4) << prob
       << ", \"is_threat\": " << (is_threat ? "true" : "false")
       << ", \"bytes\": " << bytes << ", \"packets\": " << packets
       << ", \"attention_weight\": " << std::fixed << std::setprecision(4) << prob
       << ", \"timestamp\": \"" << ts.str() << "\"}\n";
        ...
        if (now - last_scan_ns >= 1000000000ULL) {
            ...
            for (auto& pair : flow_tracker) {
                ...
                if (raw_pps > 100.0f && raw_syn_ratio > 0.8f) {
                    ...
                    write_cpp_alert_to_log(src_ip, ctx.latest_key.dst_ip, 0.98f, ctx.latest_metrics.bytes, ctx.latest_metrics.packets);
                ...
                if (ctx.packet_count >= SEQ_LEN) {
                    ...
                    write_cpp_alert_to_log(src_ip, ctx.latest_key.dst_ip, 0.0f, ctx.latest_metrics.bytes, ctx.latest_metrics.packets);
                }
            }
            ...
            if (N > 0) {
                ...
                try {
                    ...
                    for (int i = 0; i < N; ++i) {
                        ...
                        write_cpp_alert_to_log(src_ip, ctx.latest_key.dst_ip, prob, ctx.latest_metrics.bytes, ctx.latest_metrics.packets);
                    }
